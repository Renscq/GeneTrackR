...
- strand:
  ...
  Strand selector. Use `auto` to use the gene strand.
...
- show_gene_model:
  ...
  Whether to append the gene model track.
...
group-level coloring and replicate summaries. When raw intervals differ
among samples, set `bin_size` before using `signal_summary` so the
summary is made on comparable bins. `signal_transform` changes the
plotted y value only; the queried signal table is not modified. See also
