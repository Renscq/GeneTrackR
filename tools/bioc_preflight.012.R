...
# Date: 2026-09-01
# Version: dev012
# Function: Formal 0.99.1 Bioconductor submission preflight for the GeneTrackR source tree
...
if (!identical(version, "0.99.1")) {
  add_fail(paste0("Current GeneTrackR submission candidate must be 0.99.1; found ", version, "."))
} else {
  add_pass("Current submission candidate version is 0.99.1.")
}
...
if (!identical(unname(desc[["Date"]]), "2026-09-01")) {
  add_fail("DESCRIPTION Date must be 2026-09-01 for the 0.99.1 candidate.")
} else {
  add_pass("DESCRIPTION Date matches the 0.99.1 candidate date.")
}
...
for (file in vignette_files) {
  ...
  if (!grepl('author: "Rensc"', text, fixed = TRUE) ||
...
if (!file.exists(news_file)) {
  ...
  if (!grepl("# GeneTrackR 0.99.1", news_text, fixed = TRUE)) {
    add_fail("NEWS.md does not contain the current GeneTrackR 0.99.1 submission entry.")
  } else {
