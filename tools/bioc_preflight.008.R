...
# Date: 2026-08-31
# Version: dev008
# Function: Submission-hygiene and BiocCheck preflight for the GeneTrackR source tree
...
if (!file.exists(news_file)) {
  ...
  if (!grepl("# GeneTrackR 0.8.7", news_text, fixed = TRUE)) {
    add_fail("NEWS.md does not contain the current GeneTrackR 0.8.7 release entry.")
  } else {
    add_pass("NEWS.md is present and contains the current release entry.")
  }
}
...
# 0.8.7 documentation/regression hotfix checks
...
if (grepl("R (>= 4.5.0)", desc[["Depends"]], fixed = TRUE)) {
  add_pass("DESCRIPTION uses the Bioconductor R dependency floor R >= 4.5.0.")
} else {
  add_fail("DESCRIPTION should depend on R >= 4.5.0 for the current Bioconductor baseline.")
}
