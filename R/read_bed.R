...
read_bed <- function(file,
  ...
  stage <- make_stage_progress(total = 4L, progress = progress)
  ...
  dt <- data.table::fread(file, header = FALSE, sep = "\t", data.table = TRUE, showProgress = verbose)
  stage$tick()
  stop_if_not(ncol(dt) >= 3L, "A BED file must contain at least 3 columns.")
  ...
  dt[, "level" := "feature"]
  dt[, "score" := if (ncol(dt) >= 5L) suppressWarnings(as.numeric(dt[["V5"]])) else NA_real_]
  dt[, "strand" := if (ncol(dt) >= 6L) as.character(dt[["V6"]]) else "*"]
  dt[, "source" := "BED"]
