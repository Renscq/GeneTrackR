...
- Restored `NEWS.md` as a maintained package-level release history because Bioconductor strongly recommends NEWS files and BiocCheck includes dedicated NEWS checks.
- Restored the pkgdown News navigation entry and the submission-tree workflow so release notes remain available in both development documentation and the package-only Bioconductor source tree.
- Updated the Bioconductor preflight and submission preparation tools to require and preserve `NEWS.md`.
- Kept GeneTrackR analysis code, public APIs, statistical behavior, and genomic I/O unchanged from 0.8.4.

# GeneTrackR 0.8.4

## R CMD check cleanup

- Resolved the remaining package-owned `R CMD check` warning by documenting all arguments exposed in exported function usage sections.
- Declared data.table and ggplot2 non-standard-evaluation symbols reported by `R CMD check`, eliminating undefined-global-variable diagnostics without changing runtime behavior.
- Wrapped long Rd examples so generated manual examples no longer exceed the 100-character width check.
- Standardized the package license declaration to `GPL-3` and removed the redundant top-level `LICENSE` file.
- The Windows-only `future file timestamps` NOTE remains an environment time-verification issue rather than a GeneTrackR source problem.

# GeneTrackR 0.8.3

## Bioconductor submission hygiene

- Separate the development workspace from the package-only Bioconductor submission tree.
- Add a development-side submission-tree preparation tool and expand the Bioconductor preflight checks for default-branch hygiene.
- Label every R code chunk in all package vignettes for current BiocCheck compatibility.
- Document bundled `inst/extdata` files and their deterministic provenance in a package help topic.
- Update README installation guidance to lead with the Bioconductor installation path while retaining GitHub development instructions.
- Keep GeneTrackR analysis APIs and executable package behavior unchanged.

# GeneTrackR 0.8.2

- Hardened package documentation for Bioconductor build/check workflows without changing analysis algorithms or public APIs.
- Added author/date metadata, explicit Introduction and Installation sections, package landing-page links, and executable session information to all 15 formal vignettes.
- Numbered all 15 `VignetteIndexEntry` values so `browseVignettes()` follows the intended GeneTrackR workflow order instead of filename alphabetic order.
- Added build-aware vignette rendering: normal R package builds use 144 dpi to reduce CPU, memory, and artifact size, while the pkgdown workflow sets `GENETRACKR_PKGDOWN=true` to retain 300-dpi website figures.
- Kept publication export examples capable of 300-500 dpi output while executing vignette figure exports at the build-aware documentation DPI.
- Replaced all `\dontrun{}` examples in GeneTrackR roxygen/man documentation with compact, deterministic, runnable examples based on bundled `gtr_demo_*` data and temporary output paths.
- Reduced expensive signal examples to the compact RNA-seq demo where frame-specific Ribo-seq behavior was not essential to the function being documented.
- Added `tools/bioc_preflight.003.R` to audit runnable examples, vignette structure, documentation DPI policy, individual file size, estimated source-tree size, and R-build exclusion of development-only directories.
